...
// جلب بيانات الأسهم الأمريكية من Yahoo Finance
...
export default async function handler(req, res) {
  ...
  // الأسهم الأمريكية — بدون إضافة .SR
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!r2.ok) return res.status(r2.status).json({ error: `Yahoo Finance error: ${r2.status}` });
